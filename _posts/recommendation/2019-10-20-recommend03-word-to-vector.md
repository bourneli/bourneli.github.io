...
![](img\shrunking_context_size.png)
...
其中c表示center，即中心词，o表示outside，即中心词之外被预测的词，V是整个词的集合，同一个词此作为c（目标词向量）或o（softmax层对应的向量）时需要学习的向量不同。通过一顿操作（可参考CS223n课程第二讲），该函数梯度为
